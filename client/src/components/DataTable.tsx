  ...
  return (
    <div className="overflow-hidden rounded-lg border border-line bg-white">
      <div className="overflow-x-auto">
        <table className="min-w-[560px] divide-y divide-line text-sm sm:min-w-full">
          ...
          <tbody className="divide-y divide-line">
            {rows.map((row, index) => (
              <tr key={index} className="hover:bg-field">
                {row.map((cell, cellIndex) => <td key={cellIndex} className="max-w-72 whitespace-nowrap px-4 py-3">{cell}</td>)}
              </tr>
            ))}
          </tbody>
        </table>
